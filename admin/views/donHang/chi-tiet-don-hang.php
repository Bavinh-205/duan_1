<?php
// Ensure $donHang is set and valid
...
// Fix for array access warnings
$tenNguoiNhan = $donHang['ten_nguoi_nhan'] ?? 'N/A';
$emailNguoiNhan = $donHang['email_nguoi_nhan'] ?? 'N/A';
$sdtNguoiNhan = $donHang['sdt_nguoi_nhan'] ?? 'N/A';
$diaChiNguoiNhan = $donHang['dia_chi_nguoi_nhan'] ?? 'N/A';

// Validate numeric fields for number_format()
$tongTien = isset($donHang['tong_tien']) && is_numeric($donHang['tong_tien']) ? $donHang['tong_tien'] : 0;

// Handle state color logic
...
<div class="alert alert-<?= $colorAlerts ?>" role="alert">
    Đơn hàng: <?= htmlspecialchars($donHang['ten_trang_thai']) ?>
</div>

<div>
    <h2>Quản lý danh sách đơn hàng - Mã đơn: <?= htmlspecialchars($donHang['ma_don_hang']) ?></h2>
    <p>Tổng tiền: <?= number_format($tongTien, 0, ',', '.') ?>đ</p>
</div>

<!-- Table: Thông tin người nhận -->
<table>
    <tr><th>Họ tên</th><td><?= htmlspecialchars($tenNguoiNhan) ?></td></tr>
    <tr><th>Email</th><td><?= htmlspecialchars($emailNguoiNhan) ?></td></tr>
    <tr><th>Số điện thoại</th><td><?= htmlspecialchars($sdtNguoiNhan) ?></td></tr>
    <tr><th>Địa chỉ</th><td><?= htmlspecialchars($diaChiNguoiNhan) ?></td></tr>
</table>
